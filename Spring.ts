...
export default class Spring {
  ...
  public Step(deltaTime: number): number {
    ...
    if (dampingRatio === 1) {
      ...
      if (c > EPS) {
        ...
        z =
          frequencyStep +
          (((frequencyStep ** 2 * cSquared * cSquared) / 20 - cSquared) *
            frequencyStep ** 3) /
            6;
      }
      ...
      if (cFrequency > EPS) {
        ...
        y =
          deltaTime +
          (((deltaTime ** 2 * cFrequencySquared * cFrequencySquared) / 20 -
            cFrequencySquared) *
            deltaTime ** 3) /
            6;
      }
      ...
      const goalDistance = position - goal;
      const newPosition =
        (goalDistance * (i + z * dampingRatio) + velocity * y) * q + goal;
      const newVelocity =
        (velocity * (i - z * dampingRatio) - goalDistance * (z * frequency)) *
        q;
  ...
  public CanSleep(): boolean {
    return this.Velocity ** 2 > SLEEP_VELOCITY_SQ_LIMIT ||
